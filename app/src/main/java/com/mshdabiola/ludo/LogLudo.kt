...
class LogLudo(soundInterface: SoundInterface) :
    LudoGame(soundInterface = soundInterface) {
     ...
     var firebaseLog: (String, String) -> Unit = { _, _ -> }
    ...
    override suspend fun start(
        ...
        super.start(ludoGameState, ludoSetting, isGameFinish, newOnGameFinish, onPlayerFinishPlaying)
    ...
    override fun onPawnFinish(pawnIndex: Int) {
        ...
        //early bird
        if (isHumanPlaying()&&getHumanPawn().all { it.isHome().not() } && getOppPawn().all { it.isHome() }) {
